...
local pvpIconBlizzard = function(self, event, unit)
...
local CreateLargeFrame = function(self, unit)
	...
	do -- setup icons, and text
		...
		self.PvPIndicator.Override = pvpIconBlizzard
...
local CreateUnitFrameParty = function(self, unit)
	if SUI.DB.Styles.Blizzard.PartyFrames.FrameStyle == 'small' then
		self = CreateSmallFrame(self, unit)
	elseif SUI.DB.Styles.Blizzard.PartyFrames.FrameStyle == 'medium' then
		self = CreateMediumFrame(self, unit)
	elseif SUI.DB.Styles.Blizzard.PartyFrames.FrameStyle == 'large' then
...
local CreateUnitFrameRaid = function(self, unit)
	if SUI.DB.Styles.Blizzard.RaidFrames.FrameStyle == 'small' then
		self = CreateSmallFrame(self, unit)
	elseif SUI.DB.Styles.Blizzard.RaidFrames.FrameStyle == 'medium' then
		self = CreateMediumFrame(self, unit)
	elseif SUI.DB.Styles.Blizzard.RaidFrames.FrameStyle == 'large' then
		self = CreateLargeFrame(self, unit)
	end
...
SpartanoUF:RegisterStyle('Spartan_BlizzardPlayerFrames', CreateUnitFrame)
SpartanoUF:RegisterStyle('Spartan_BlizzardPartyFrames', CreateUnitFrameParty)
SpartanoUF:RegisterStyle('Spartan_BlizzardRaidFrames', CreateUnitFrameRaid)
...
function module:PlayerFrames()
	Images = {
		Alliance = {
			bg = {
				Texture = 'Interface\\addons\\SpartanUI_Style_Blizzard\\Images\\UnitFrames',
	...
	SpartanoUF:SetActiveStyle('Spartan_BlizzardPlayerFrames')
	...
	Blizzard_SpartanUI:HookScript(
		...
		function(this, event)
			if unattached then
				SUI_FramesAnchor:SetParent(Blizzard_SpartanUI)
				module:PositionFrame()
			end
		end
	)
...
function module:PositionFrame(b)
	...
	--Set Position
	if Blizzard_SpartanUI.Left then
		if b == 'player' or b == nil then
			PlayerFrames.player:SetPoint('BOTTOMRIGHT', Blizzard_SpartanUI.Left, 'TOPLEFT', -60, 10)
		end
	else
		if b == 'player' or b == nil then
...
function module:RaidFrames()
	SpartanoUF:SetActiveStyle('Spartan_BlizzardRaidFrames')
	module:RaidOptions()
	...
	if SUI.DB.Styles.Blizzard.RaidFrames.FrameStyle == 'medium' then
		xoffset = 10
	end
	...
	local raid =
		...
		module:FrameSize(SUI.DB.Styles.Blizzard.RaidFrames.FrameStyle)
	)
...
function module:PartyFrames()
	...
	SpartanoUF:SetActiveStyle('Spartan_BlizzardPartyFrames')
	module:PartyOptions()
	...
	local party =
		...
		'TOPLEFT',
		'oUF-initialConfigFunction',
		module:FrameSize(SUI.DB.Styles.Blizzard.PartyFrames.FrameStyle)
	)
...
function module:RaidOptions()
	SUI.opt.args['RaidFrames'].args['FrameStyle'] = {
		...
		get = function(info)
			return SUI.DB.Styles.Blizzard.RaidFrames.FrameStyle
		end,
		set = function(info, val)
			SUI.DB.Styles.Blizzard.RaidFrames.FrameStyle = val
			SUI:reloadui()
		end
	}
end
...
function module:PartyOptions()
	SUI.opt.args['PartyFrames'].args['FrameStyle'] = {
		...
		values = {['large'] = L['Large'], ['medium'] = L['Medium'], ['small'] = L['Small']},
		get = function(info)
			return SUI.DB.Styles.Blizzard.PartyFrames.FrameStyle
		end,
		set = function(info, val)
			SUI.DB.Styles.Blizzard.PartyFrames.FrameStyle = val
			SUI:reloadui()
		end
	}
end
